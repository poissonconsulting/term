...
#'
#' Validates the elements of a term_rcrd object.
#' Use [chk_s3_class()] to check if an object is a term_rcrd.
#'
#' Internal validity of a term can be checked on three levels:
#'
#' - `"valid"` checks that all terms are of the form
...
vld_term_rcrd <- function(x, validate = "complete") {
  ...
  x <- x[!is.na(x$par) & !is.na(x$dim), , drop = FALSE]
  if (!nrow(x)) {
    return(TRUE)
  }
