context("repair-terms")

test_that("repair_terms", {
  expect_identical(repair_terms(term(0)), term(0))
  expect_identical(repair_terms(NA_character_), NA_term_)
  expect_identical(repair_terms(as.term("")), term(0))
  expect_identical(repair_terms(as.term("[1]")), term(0))
  expect_identical(repair_terms(as.term("b[]")), term(0))
  expect_identical(repair_terms(as.term("b[0]")),term(0))
  expect_identical(repair_terms(as.term("b[1,]")), term())
  expect_identical(repair_terms(as.term("a")), as.term("a"))
  expect_identical(repair_terms(as.term("a ")), as.term("a"))
  expect_identical(repair_terms(as.term("a [2]")), as.term("a[2]"))
  expect_identical(repair_terms(as.term(c("b [  3 ]", "a [2]"))), 
                   as.term(c("b[3]", "a[2]")))
  expect_identical(repair_terms(as.term(c("a", ""))), as.term("a"))
  expect_identical(repair_terms(as.term(c("a[1]", ""))), as.term("a"))
  expect_identical(repair_terms(as.term(c("a[2]", "a[1]"))), as.term(c("a[2]", "a[1]")))
  expect_identical(repair_terms(as.term(c("a[2,1]", "a"))), as.term(c("a[2,1]", "a[1]")))
})
