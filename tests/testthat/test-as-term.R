...
test_that("as.term.integer", {
  ...
  expect_identical(as.term(c(1L,4L), name = "par"), 
                   as.term(c("par[1]", "par[2]")))
  expect_identical(as.term(integer(0), name = "par"), 
                   as.term(character(0)))
})

test_that("as.term.double", {
  expect_identical(as.term(1, name = "par"), as.term("par"))
  expect_identical(as.term(c(1,4), name = "par"), 
                   as.term(c("par[1]", "par[2]")))
  expect_identical(as.term(double(0), name = "par"), 
                   as.term(character(0)))
})

test_that("as.term.matrix", {
  expect_identical(as.term(matrix(1)[-1,-1], "t"),
                   as.term(character(0)))
  expect_identical(as.term(matrix(1:12, c(3,4)), "t"),
                   structure(c("t[1,1]", "t[2,1]", "t[3,1]", "t[1,2]", "t[2,2]", 
"t[3,2]", "t[1,3]", "t[2,3]", "t[3,3]", "t[1,4]", "t[2,4]", "t[3,4]"
), class = c("term", "character")))
})

test_that("as.term.array", {
  expect_identical(as.term(array(1,c(1,1,1))[-1,-1,-1], "t"),
                   as.term(character(0)))
  expect_identical(as.term(array(1:12, c(2,3,2)), "t"),
                   structure(c("t[1,1,1]", "t[2,1,1]", "t[1,2,1]", "t[2,2,1]", "t[1,3,1]", 
